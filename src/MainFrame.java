...
public class MainFrame extends JPanel implements ActionListener {
	...
	public MainFrame() {
		...
		registerPanel
				.add(new JLabel(
						"<html>Aren't registered as a tutor or tutee?<br/>Pick your role above and click<br/>the"
								+ " Register button.     </html>    "));
		registerPanel.add(registerButton);

		mainScreen.add(new JLabel(
				"Welcome to the Tutor Matcher! Please log in.    ",
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (event.getSource().equals(registerButton)) {
			...
			try {
				...
				if (found) {
					MatcherMain.setOptionsFrame(subject, fnameField.getText(),
							lnameField.getText(), emailField.getText(),
							tuteeChoice.isSelected());
					this.setVisible(false);
					parent.remove(this);
