...
public class TutorTableFrame extends JPanel implements ActionListener {
	...
	private void findTutors() {
		...
		ArrayList<String> lessthan10info = new ArrayList<String>();
		
		try {
			for (String s:fileInfo) {
				br = new BufferedReader(new FileReader(s.split(",")[0]+" "+s.split(",")[1]+".csv"));
				String currentLine = br.readLine();
				float totalMinutes = 0;
				while (currentLine != null) {
					totalMinutes += Float.parseFloat(currentLine.split(",")[2]);
					currentLine = br.readLine();
				}
				
				if (!(totalMinutes/60<10)) {
					lessthan10info.add(currentLine);
				}
			}
		} catch (Exception ex) {
		}
		
		if (lessthan10info.size()==0) {
			lessthan10info = fileInfo;
		}
		
		tutorInfo = new String[lessthan10info.size()][5];

		int index = 0;
		for (String i : lessthan10info) {
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (event.getSource().equals(selectTutorButton)) {
			// checking to see if tutor has already been requested
			...
			try {
				...
				while (currentLine != null) {
					if (currentLine
							.contains(tutorInfo[table.getSelectedRow()][0]
									+ ","
									+ tutorInfo[table.getSelectedRow()][1]
									+ ","
									+ tutorInfo[table.getSelectedRow()][2])) {
			...
			if (!alreadyRequested) {
				...
				try {
					...
					String writeInfo = tutorInfo[table.getSelectedRow()][0]
							+ ","
							+ tutorInfo[table.getSelectedRow()][1]
							+ ","
							+ tutorInfo[table.getSelectedRow()][2]
				...
				JOptionPane.showMessageDialog(TableScreen, "This tutor is currently being requested.\nYou may come back later" +
						"to see whether they are available\nor select a different tutor.");
			}

		} else if (event.getSource().equals(backToOptionsButton)) {
			MatcherMain
					.setOptionsFrame(subject, fname, lname, tuteeEmail, true);
